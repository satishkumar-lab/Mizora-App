...
const ROW_H_PAD = 18;
export const UNLOCK_REWARDS_CARD_PAD = 18;
...
export type UnlockRewardsListCardProps = {
  apps: RewardAppItem[];
...
function ManageBlockedAppsButton({ onPress }: { onPress?: () => void }) {
  const { colors } = useMizoraTheme();
  return (
    <View style={{ paddingTop: 14, paddingBottom: UNLOCK_REWARDS_CARD_PAD + 2 }}>
      <View
        className="h-px dark:bg-[#2a332a]"
        style={{
          marginHorizontal: UNLOCK_REWARDS_CARD_PAD,
          marginBottom: 14,
      ...
      <Pressable
        ...
        style={({ pressed }) => ({
          paddingVertical: 16,
          paddingHorizontal: UNLOCK_REWARDS_CARD_PAD,
    ...
    >
      <View
        style={{
          ...
          paddingTop: UNLOCK_REWARDS_CARD_PAD + 2,
          paddingBottom: showManageFooter ? 6 : UNLOCK_REWARDS_CARD_PAD + 2,
        }}
      >
        {apps.map((app, index) => (
          <View key={app.id}>
